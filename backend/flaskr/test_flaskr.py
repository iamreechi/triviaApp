...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.database_path = 'postgresql://{}@{}/{}'.format('postgres:abc','localhost:5432', self.database_name)#'postgresql://postgres:abc@localhost:5432/trivia'
    ...
    def test_delete_specific_question(self):
        ...
        self.assertEqual(data['question'], 24)
    ...
    def test_search_question(self):
        res = self.client().post('/questions',
                                 json={'searchTerm': 'the Hall of Mirrors'})
        data = json.loads(res.data.decode('utf-8'))

        self.assertEqual(res.status_code, 200)
        self.assertTrue(data['questions'])
        self.assertEqual(len(data['questions']), len(Question.query.order_by(Question.id).filter(Question.question.ilike('%{}%'.format('the Hall of Mirrors'))).all()))
